...
object StorageManager {
    ...
    fun lockFile(context: Context, fileName: String, cameraType: String) {
        val resolver = context.contentResolver
        val selection = "${MediaStore.Video.Media.DISPLAY_NAME} = ?"
        val selectionArgs = arrayOf(fileName)

        val values = ContentValues().apply {
            put(MediaStore.Video.Media.DISPLAY_NAME, fileName.replace(".mp4", "_LOCKED.mp4"))
        }

        try {
            resolver.update(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, values, selection, selectionArgs)
            Log.d(TAG, "File locked: $fileName")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to lock file", e)
        }
    }
